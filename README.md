...
│  │   ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌────────────┐        │   │
│  │   │  综合选股  │  │  指标计算  │  │  形态识别  │  │  策略选股  │        │   │
│  │   │ selection  │  │ indicator  │  │  pattern   │  │  strategy  │        │   │
│  │   └─────┬──────┘  └─────┬──────┘  └─────┬──────┘  └─────┬──────┘        │   │
│  │         │               │               │               │                │   │
│  │   ┌─────┴───────────────┴───────────────┴───────────────┴─────┐          │   │
│  │   │                                                           │          │   │
...
instock/
├── bin/                          # 启动脚本
│   ├── run_job.bat/.sh           #   数据作业脚本
│   ├── run_web.bat/.sh           #   Web服务启动
│   └── run_trade.bat             #   交易服务启动
│
├── config/                       # 配置文件
│   ├── proxy.txt                 #   代理配置
│   ├── eastmoney_cookie.txt      #   Cookie配置
│   └── trade_client.json         #   交易账户配置
│
├── core/                         # 核心业务模块
│   ├── crawling/                 #   数据爬取
│   │   ├── stock_hist_em.py      #     历史行情
│   │   ├── stock_fund_em.py      #     资金流向
│   │   ├── stock_lhb_em.py       #     龙虎榜
│   │   └── ......                #     其他数据源
│   │
│   ├── indicator/                #   指标计算
│   │   └── calculate_indicator.py#     32种技术指标
│   │
│   ├── pattern/                  #   K线形态识别
...
│   ├── backtest/                 #   回测模块
│   ├── kline/                    #   K线可视化
│   ├── stockfetch.py             #   数据获取统一接口
│   ├── tablestructure.py         #   表结构元数据
│   └── singleton_*.py            #   单例数据缓存
│
├── job/                          # 定时任务
│   ├── execute_daily_job.py      #   主作业入口
│   ├── basic_data_daily_job.py   #   基础数据作业
│   ├── indicators_data_daily_job.py# 指标计算作业
│   ├── strategy_data_daily_job.py#   策略选股作业
│   └── backtest_data_daily_job.py#   回测作业
│
├── lib/                          # 基础设施
│   ├── database.py               #   数据库连接
│   ├── singleton_type.py         #   单例元类
│   ├── trade_time.py             #   交易时间工具
│   └── run_template.py           #   批量作业模板
│
├── trade/                        # 交易系统
│   ├── robot/                    #   交易引擎
│   │   └── engine/               #     事件驱动引擎
...
│  │   │                                                               │     │   │
│  │   │   ┌─────────────┐   ┌─────────────┐   ┌─────────────┐        │     │   │
│  │   │   │ stagging.py │   │ strategy1.py│   │ strategy2.py│  ...   │     │   │
│  │   │   │  (打新策略)  │   │  (自定义)   │   │  (自定义)   │        │     │   │
│  │   │   └──────┬──────┘   └──────┬──────┘   └──────┬──────┘        │     │   │
│  │   │          │                 │                 │                │     │   │
│  │   │          └─────────────────┼─────────────────┘                │     │   │
...
│                                                                                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│  │   Python    │  │   MySQL     │  │   Docker    │  │  Supervisor │            │
│  │    3.11     │  │  /MariaDB   │  │             │  │             │            │
│  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘            │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │                           Python 依赖库                                  │   │
│  │                                                                         │   │
│  │  数据处理        技术分析        Web框架         交易接口                 │   │
│  │  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐               │   │
│  │  │ pandas  │    │ TA-Lib  │    │ tornado │    │easytrader│              │   │
│  │  │ numpy   │    │         │    │ bokeh   │    │         │               │   │
│  │  └─────────┘    └─────────┘    └─────────┘    └─────────┘               │   │
│  │                                                                         │   │
│  │  数据库          网络请求        工具库                                   │   │
│  │  ┌─────────┐    ┌─────────┐    ┌─────────┐                              │   │
│  │  │SQLAlchemy│   │requests │    │  arrow  │                              │   │
│  │  │ PyMySQL │    │   bs4   │    │  tqdm   │                              │   │
│  │  └─────────┘    └─────────┘    └─────────┘                              │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
...
```
   ...
   ┌──────────────┐          ┌──────────────┐          ┌──────────────┐
   │ 实时行情数据 │          │ 完整数据作业 │          │  历史数据    │
   │  basic_data  │          │execute_daily │          │   清理/归档  │
   └──────────────┘          └──────────────┘          └──────────────┘
                                    │
                   ┌────────────────┼────────────────┐
                   │                │                │
